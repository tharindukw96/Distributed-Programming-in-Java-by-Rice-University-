...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.join(ranks).flatMapToPair(kv->{
            ...
            Tuple2<Website, Double> values = kv._2();

            Website edges = kv._2()._1();
            Double currentRank = kv._2()._2();

            List<Tuple2<Integer, Double>> contributes = new LinkedList<Tuple2<Integer, Double>>();
            ...
            while(iterator.hasNext()){
                ...
                contributes.add(new Tuple2(target, currentRank / (double) edges.getNEdges()));
            }
           return contributes;
        });
